...
class Basic2dTrans(nn.Module, ABC):     # 反卷积上采样到2倍分辨率 
    ...
    def forward(self, x):
        out = self.conv(x)  # (n,n) ---> (2n, 2n)
        out = self.bn(out)
        out = self.relu(out)
        return out

class BasicBlock(nn.Module, ABC):   # 两个3*3卷积后，残差连接
    expansion = 1
...
class BasicDepthEncoder(nn.Module, ABC):
    ...
    def forward(self, depth, context):
        ...
        d2 = self.convd2(d1)    # depth (self-guided feature) (B, 128, H, W)

        f1 = self.convf1(context)
        f2 = self.convf2(f1)    # cross-guided feature (B, 128, H, W)

        input_feature = torch.cat((d2, f2), dim=1)  # (B, 256, H, W）
        input_feature = self.conv(input_feature)
        feature = self.ref(input_feature)
        weight = torch.sigmoid(self.conv_weight(feature))   # (B, K^2, H, W)
        offset = self.conv_offset(feature)

        # Add zero reference offset
        offset = offset.view(B, self.num, 2, H, W)      # 将原先通道位2*(k^2-1) reshape 成（k^2-1, 2）表示两个方向的偏移量 , (B, num, 2, H, W)
        list_offset = list(torch.chunk(offset, self.num, dim=1))    # 获得num个(B, 1, 2, H, W)张量组成的list
        list_offset.insert(self.idx_ref, torch.zeros((B, 1, 2, H, W)).type_as(offset))  # 将0插入list中间
        offset = torch.cat(list_offset, dim=1).view(B, -1, H, W)    # 恢复成(B, K^2, 2, H, W)的张量
...
class Post_process_deconv(nn.Module, ABC):
    ...
    def forward(self, depth, weight, offset):
        ...
        if self.dkn_residual:
            weight = weight - torch.mean(weight, 1).unsqueeze(1).expand_as(weight)  # (B, K^2, H, W)
        else:
