...
class Transforms(object):
    ...
    def __init__(self,
        ...
        self.do_random_remove_points = True if random_remove_points else False
        self.remove_points_range = random_remove_points

        self.do_random_noise = True if (random_noise_type and random_noise_spread > 0) else False
    ...
    def transform(self,
        ...
        Arg(s):
            images_arr : list[torch.Tensor]
                list of N x C x H x W tensors   [image0, image1, image2]
        ...
        do_random_transform = np.random.rand(n_batch) <= random_transform_probability

        if self.do_random_horizontal_flip:
            do_horizontal_flip = np.logical_and(do_random_transform, np.random.rand(n_batch) <= 0.50)
            images_arr = self.horizontal_flip(images_arr, do_horizontal_flip)
            range_maps_arr = self.horizontal_flip(range_maps_arr, do_horizontal_flip)
            validity_maps_arr = self.horizontal_flip(validity_maps_arr, do_horizontal_flip)

        if self.do_random_vertical_flip:
            do_vertical_flip = np.logical_and(do_random_transform, np.random.rand(n_batch) <= 0.50)
            images_arr = self.vertical_flip(images_arr, do_vertical_flip)
            range_maps_arr = self.vertical_flip(range_maps_arr, do_vertical_flip)
            validity_maps_arr = self.vertical_flip(validity_maps_arr, do_vertical_flip)

        if self.do_random_remove_points:
            do_remove_points = np.logical_and(do_random_transform, np.random.rand(n_batch) <= 0.50)
            values = torch.rand(n_batch, device=device)
            remove_points_min, remove_points_max = self.remove_points_range
            densities = (remove_points_max - remove_points_min) * values + remove_points_min    # 取min～max之间的随机值
            range_maps_arr = self.remove_random_nonzero(images_arr=range_maps_arr, do_remove=do_remove_points, densities=densities)     # 只处理了sparse_depth，没处理validitymap?

        if self.do_random_noise:
            do_add_noise = np.logical_and(do_random_transform, np.random.rand(n_batch) <= 0.50)
            range_maps_arr = self.add_noise(range_maps_arr, do_add_noise=do_add_noise, noise_type=self.random_noise_type, noise_spread=self.random_noise_spread)

        # Return the transformed inputs
        outputs = {}
        if len(images_arr) > 0:
            outputs['images_arr'] = images_arr
        if len(range_maps_arr) > 0:
            outputs['range_maps_arr'] = range_maps_arr
        if len(validity_maps_arr) > 0:
            outputs['validity_maps_arr'] = validity_maps_arr
    ...
    def random_nonzero(self, T, density=0.10):
        ...
        random_subset = torch.randperm(nonzero_indices[0].shape[0], device=T.device)    # 数量
