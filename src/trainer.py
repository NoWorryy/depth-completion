    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if (iteration % train_params['n_summary']) == 0:
                ...
                # 保存图像 这里都是(b, c, h, w)
                sparse_depth0_color = colorize((inputs['sparse_depth0'] / model_params['max_predict_depth']).cpu(), colormap='viridis')
                output_depth0_color = colorize((generated['output_depth0'] / model_params['max_predict_depth']).cpu(), colormap='viridis')
                
                train_data_pair = torch.cat([
                    sparse_depth0_color, inputs['image0'], output_depth0_color, generated['image01'], generated['iamge02']],
                    dim = 2)
                torchvision.utils.save_image(train_data_pair, f"{output_dir}/output_image/{f'train_data_pair-{iteration}'}.png")
                tb_writer.add_images("train_sparse_image_dense_image01_image02", train_data_pair, global_step=iteration)
                ...
                for idx, inputs in enumerate(val_dataloader):
                    ...
                    if idx == 0:    # 保存第一个batch的图片
                        sparse_depth_color = colorize((inputs['sparse_depth'] / model_params['max_predict_depth']).cpu(), colormap='viridis')
                        output_depth_color = colorize((generated_val['output_depth'] / model_params['max_predict_depth']).cpu(), colormap='viridis')
                        ...
                        val_data_pair = torch.cat([
                            ...
                            dim = 2)
                        torchvision.utils.save_image(val_data_pair, f"{output_dir}/output_image/{f'train_data_pair-{iteration}'}.png")
                        tb_writer.add_images("val_sparse_image_dense_error", val_data_pair, global_step=iteration)
