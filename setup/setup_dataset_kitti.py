...
def process_frame(inputs):
    ...
    if not paths_only:
        ...
        image = np.concatenate([image1, image0, image2], axis=1)    # 前一帧、当前帧、后一帧

        _, validity_map = data_utils.load_depth_with_validity_map(sparse_depth_path)    # 稀疏深度图和置信度图
    ...
    if not paths_only:
        # Write to disk
        data_utils.save_validity_map(validity_map, validity_map_output_path)    # 保存置信度图，有效值为256，无效值0
        cv2.imwrite(image_output_path, image)   # 保存三个相邻帧图像
...
intrinsics_files = sorted(glob.glob(os.path.join(
    KITTI_RAW_DATA_DIRPATH, '*', KITTI_CALIBRATION_FILENAME)))  # 读取kitti_raw的左右目标定文件
intrinsics_dkeys = {}
...
for intrinsics_file in intrinsics_files:
    ...
    if not args.paths_only:
        ...
        intrinsics2 = np.reshape(calib['P_rect_02'], [3, 4])    # 读取相机内参（投影矩阵）
    ...
    sequence_date = intrinsics_file.split(os.sep)[2]    # '2011_09_26'
...
for refdir in ['train', 'val']:
    sparse_depth_sequence_dirpath = glob.glob(
        os.path.join(KITTI_TRAINVAL_SPARSE_DEPTH_DIRPATH, refdir, '*/'))    # 读取sparse_depth下的目录,
    ...
    for sequence_dirpath in sparse_depth_sequence_dirpath:
        ...
        for camera_dirpath in ['image_02', 'image_03']:
            ...
            sparse_depth_paths = sorted(glob.glob(
                os.path.join(
                    sequence_dirpath, KITTI_TRAINVAL_SPARSE_DEPTH_REFPATH, camera_dirpath, '*.png')))   # 读取sparse_depth图片
            ...
            ground_truth_paths = sorted(glob.glob(
                os.path.join(
                    ground_truth_sequence_dirpath, KITTI_TRAINVAL_GROUND_TRUTH_REFPATH, camera_dirpath, '*.png')))  # 读取ground_truth图片

            assert len(sparse_depth_paths) == len(ground_truth_paths)

            # Obtain sequence dirpath in raw data
            sequence = sparse_depth_paths[0].split(os.sep)[5]   # '2011_09_26_drive_0001_sync'
            sequence_date = sequence[0:10]  # '2011_09_26'
            raw_sequence_dirpath = os.path.join(
                KITTI_RAW_DATA_DIRPATH, sequence_date, sequence, camera_dirpath, 'data')
            image_paths = sorted(
                glob.glob(os.path.join(raw_sequence_dirpath, '*.png')))     # 读取原RGB图像
            ...
            for idx in range(len(sparse_depth_paths)):
                ...
                image0_path_idx = image_paths.index(image0_path)    # raw_image是比ground_truth_depth前后多五张图片的
                image1_path = image_paths[image0_path_idx-1]        # 取前后帧的图片
...
for path in kitti_static_frames_paths:  # 2011_09_26 2011_09_26_drive_0009_sync 0000000386
    parts = path.split(' ')
    kitti_static_frames_parts.append((parts[1], parts[2]))  # (2011_09_26_drive_0009_sync, 0000000386)
...
# Write all training file paths ------ kitti_train_*.txt
...
# Write unused file paths  ------  kitti_unused_*.txt
...
print('Removed {} filepaths for clean paths'.format(n_removed))     # static

print('Storing clean training image file paths into: {}'.format(    # 不含static的 kitti_train_*-clean.txt
